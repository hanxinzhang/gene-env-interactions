...
with basic_model:
    ...
    # G*F effect
    gf_shrinkage = pm.HalfCauchy('gf_shrinkage', beta=1.0)
    gf_scale = pm.Normal('gf_scale', mu=0, sigma=gf_shrinkage)
    gf = gf_scale * g * f
    
    # G*C effect
    gc_shrinkage = pm.HalfCauchy('gc_shrinkage', beta=1.0)
    gc_scale = pm.Normal('gc_scale', mu=0, sigma=gc_shrinkage)
    gc = gc_scale * g * c
    
    # G*S effect
    gs_shrinkage = pm.HalfCauchy('gs_shrinkage', beta=1.0)
    gs_scale = pm.Normal('gs_scale', mu=0, sigma=gs_shrinkage)
    gs = gs_scale * g * s
    
    # G*E effect
    ge_shrinkage = pm.HalfCauchy('ge_shrinkage', beta=1.0)
    ge_scale = pm.Normal('ge_scale', mu=0, sigma=ge_shrinkage)
    ge = ge_scale * g * e
    
    # Additive model
    l = tt.dot(BIAS_SEX_AGE_EQI, fixed_b) + f_gp_geo[GEO_COORD_INDEX] + g + f + c + s + e + gf + gc + gs + ge
...
pm.traceplot(trace, ['g_var', 'f_var', 'c_var', 's_var', 'e_var', 
                     'geo_ls', 'geo_var',
                     'gf_shrinkage', 'gf_scale',
                     'gc_shrinkage', 'gc_scale',
                     'gs_shrinkage', 'gs_scale',
                     'ge_shrinkage', 'ge_scale',
                     'b_shrinkage', 'fixed_b'])
...
gf_trace = (trace['gf_scale'] ** 2.0) * trace['g_var'] * trace['f_var']
gc_trace = (trace['gc_scale'] ** 2.0) * trace['g_var'] * trace['c_var']
gs_trace = (trace['gs_scale'] ** 2.0) * trace['g_var'] * trace['s_var']
ge_trace = (trace['ge_scale'] ** 2.0) * trace['g_var'] * trace['e_var']

total_var_trace = (geo_var_trace + g_var_trace + f_var_trace + c_var_trace + s_var_trace + e_var_trace + 
                   gf_trace + gc_trace + gs_trace + ge_trace)
...
hf2_trace = gf_trace / total_var_trace
hc2_trace = gc_trace / total_var_trace
hs2_trace = gs_trace / total_var_trace
he2_trace = ge_trace / total_var_trace
...
hf2_hpd = pm.hpd(hf2_trace, hdi_prob=0.95)
hc2_hpd = pm.hpd(hc2_trace, hdi_prob=0.95)
hs2_hpd = pm.hpd(hs2_trace, hdi_prob=0.95)
he2_hpd = pm.hpd(he2_trace, hdi_prob=0.95)
...
hf2_mean = hf2_trace.mean()
hc2_mean = hc2_trace.mean()
hs2_mean = hs2_trace.mean()
he2_mean = he2_trace.mean()
...
key_results = pd.DataFrame(data={'p2 mean': [p2_mean],
                                 ...
                                 'hf2 mean': [hf2_mean],
                                 'hf2 95% HPD lo': [hf2_hpd[0]],
                                 'hf2 95% HPD up': [hf2_hpd[1]],
                                 
                                 'hc2 mean': [hc2_mean],
                                 'hc2 95% HPD lo': [hc2_hpd[0]],
                                 'hc2 95% HPD up': [hc2_hpd[1]],
                                 
                                 'hs2 mean': [hs2_mean],
                                 'hs2 95% HPD lo': [hs2_hpd[0]],
                                 'hs2 95% HPD up': [hs2_hpd[1]],
                                 
                                 'he2 mean': [he2_mean],
                                 'he2 95% HPD lo': [he2_hpd[0]],
                                 'he2 95% HPD up': [he2_hpd[1]],
